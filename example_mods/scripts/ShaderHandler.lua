...
--		2. u must have these `require` and `init` in start of onCreatePost() to initialize functions
...
function removeShadersOnCamera(camera, shadersTable)
	if not initialized then return nil end
	for i, shader in ipairs(shadersTable) do
		removeShaderFromCamera(camera, shader)
	end
end
...
function setShaderValue(shader, name, value)
	...
	if not isSampler then value = {value} end
	runHaxeCode([[
...
function doTweenShaderValue(tag, shader, name, value, duration, ease)
	...
	runHaxeCode([[
		...
		game.modchartTweens.set("]]..tag..[[", FlxTween.num(shader.data.]]..name..[[.value[0], ]]..value..[[, ]]..duration..[[, {
			ease: getEase("]]..ease..[["),
			onComplete: twn -> {
