...
class AchievementsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		for (option in options)
		{
			...
			if(option.unlocked)
			{
				...
				if(graphic == null) graphic = Paths.image('placeHolder');
			}
	...
	function _changeSelection()
	{
		...
		if(hasProgress)
		{
			...
			progressTxt.text = Highscore.floorDecimal(val1, options[curSelected].decProgress) + ' / ' + Highscore.floorDecimal(val2, options[curSelected].decProgress);
