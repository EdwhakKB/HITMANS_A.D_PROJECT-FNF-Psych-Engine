...
class QuantSubState extends MusicBeatSubstate
{
	...
	var rgbQuantize:Array<Array<FlxColor>> = [
	...
	public function new() {
		...
		skinIndicator = new FlxText(260, 20, 0, ClientPrefs.data.notesSkin[0], 56);
			...
			else if (pointerOverlaps(myNotes))
			{
				myNotes.forEachAlive(function(note:StrumNote) {
					if (curSelectedNote != note.ID && pointerOverlaps(note))
					{
						...
						onModeColumn = false;
						bigNote.rgbShader.parent = Note.globalQuantRgbShaders[note.ID];
						bigNote.shader = Note.globalQuantRgbShaders[note.ID].shader;
						updateNotes();
						FlxG.sound.play(Paths.sound('scrollMenu'), 0.6);
					}
				});
	...
	public function spawnNotes()
	{
		...
		for (i in 0...dataArray.length)
		{
			...
			var newNote:StrumNote = new StrumNote(25 + (680 / dataArray.length * i), 200, i, 0, true);
	...
	function getShader() return Note.globalQuantRgbShaders[curSelectedNote];
}
