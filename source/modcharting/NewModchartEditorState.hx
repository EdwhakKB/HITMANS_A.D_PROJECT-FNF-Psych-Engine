...
class NewModchartEditorEvent extends FlxSprite
{
    ...
    public function new (data:Array<Dynamic>)
    {
        ...
        animation.play('note');
        setGraphicSize(NewModchartEditorState.gridSize, NewModchartEditorState.gridSize);
        updateHitbox();
        antialiasing = true;
    }
...
class NewModchartEditorState extends MusicBeatState
{
    ...
    public var eventSprites:FlxTypedGroup<NewModchartEditorEvent>;
    ...
    {	
        ...
        eventSprites = new FlxTypedGroup<NewModchartEditorEvent>();
        add(eventSprites);
        ...
        NoteMovement.getDefaultStrumPosNewEditor(this);
    ...
    override public function update(elapsed:Float)
    {
        ...
        if (FlxG.mouse.y < grid.y+grid.height && FlxG.mouse.y > grid.y) //not using overlap because the grid would go out of world bounds
        {
            ...
            if (FlxG.mouse.overlaps(eventSprites))
            {
                ...
                eventSprites.forEachAlive(function(event:NewModchartEditorEvent)
                {
        ...
        if (FlxG.keys.justPressed.ESCAPE)
        {
            ...
            if (hasUnsavedChanges)
            {
                ...
                openSubState(new NewModchartEditorExitSubstate(exitFunc));
            }
            else 
                exitFunc();
    ...
    function updateEventSprites()
    {
        ...
        // while (i >= 0) {
        //     var daEvent:NewModchartEditorEvent = eventSprites.members[i];
        ...
        for (i in 0...playfieldRenderer.modchart.data.events.length)
        {
            ...
            {
                var daEvent:NewModchartEditorEvent = new NewModchartEditorEvent(playfieldRenderer.modchart.data.events[i]);
...
class NewModchartEditorExitSubstate extends MusicBeatSubstate
{
