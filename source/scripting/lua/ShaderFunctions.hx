...
class ShaderFunctions
{
	public static function implement(funk:FunkinLua)
	{
		...
		funk.addLocalCallback("setSpriteShader", function(obj:String, shader:String) {
			...
			#if (!flash && sys)
			if(!funk.shaderValues.exists(shader) && !funk.initLuaShader(shader))
			{
			...
			if(leObj != null) {
				var arr:Array<String> = funk.shaderValues.get(shader);
				funk.runtimeShaders.set(shader, new shaders.ErrorHandledShader.ErrorHandledRuntimeShader(shader, arr[0], arr[1]));
				leObj.shader = funk.runtimeShaders.get(shader);
				return true;
			}
			#else
