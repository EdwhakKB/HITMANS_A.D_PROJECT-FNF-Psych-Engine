...
import states.subs.GameOverSubstate;
...
class FunkinLua {
	...
	public function new(scriptName:String) {
		try {
			...
			@:privateAccess
			{
				...
				set('chaosMod', game.chaosMod);
				set('chaosDiff', game.chaosDifficulty);
			...
			Lua_helper.add_callback(lua, "updateScoreText", function() game.updateScore());
			...
			Lua_helper.add_callback(lua, "makeLuaSkewedSprite", function(tag:String, ?image:String = null, ?x:Float = 0, ?y:Float = 0, ?skewX:Float = 0, ?skewY:Float = 0) {
				tag = tag.replace('.', '');
				LuaUtils.destroyObject(tag);
				var leSprite:FlxSkewedSprite = new FlxSkewedSprite();
				if(image != null && image.length > 0)
					leSprite.loadGraphic(Paths.image(image));
				leSprite.setPosition(x, y);
				leSprite.skew.set(skewX, skewY);
				MusicBeatState.getVariables().set(tag, leSprite);
				leSprite.active = true;
			});
			...
			if (game.notITGMod && PlayState.SONG.notITG)
	...
	public function initLuaShader(name:String)
	{
		...
		{
			var shaderData:Array<String> = shaderValues.get(name);
