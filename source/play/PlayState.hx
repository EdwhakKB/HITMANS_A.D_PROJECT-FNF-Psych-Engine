...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		final edwhakVariable:Array<String> = ['Edwhak', 'he', 'edwhakBroken', 'edkbmassacre'];
		Note.canDamagePlayer = !edwhakVariable.contains(boyfriend.curCharacter);
		Note.edwhakIsPlayer = edwhakVariable.contains(boyfriend.curCharacter);
			...
			⠀⠀⠀⠀⠁⠇⠡⠩⡫⢿⣝⡻⡮⣒⢽⠋⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
			—————————————————————————————*/
		edwhakIsEnemy = edwhakVariable.contains(dad.curCharacter);
	...
	function set_songSpeed(value:Float):Float
	{
		if(generatedMusic)
		{
			if (notITGMod && SONG.notITG && playfieldRenderer != null)
	...
	public function startCountdown():Void
	{
		...
		if(ret != LuaUtils.Function_Stop) {
			...
			if (startOnTime > 0 && startingfromCheckpoint) {
				...
				vocals.volume = 0;
				variables.set("checkpointVolumeFadeIn", FlxTween.num(0, 1, (Conductor.crochet/1000) * 10, {ease: FlxEase.sineOut, onComplete: function(twn:FlxTween) {
					variables.remove("checkpointVolumeFadeIn");
				}}, function(v){
					vocals.volume = v;
	...
	function skipIntro(?songPosToGoTo:Float=0){
		...
		FlxG.sound.music.pause();
		vocals.pause();
		Conductor.songPosition = songPosToGoTo == 0 ? introSkip * 1000 : songPosToGoTo;
		if (SONG.notITG && notITGMod && playfieldRenderer != null)
		...
		{
			DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", hitmansHUD.iconP2.getCharacter(), true, songLength - Conductor.songPosition - ClientPrefs.data.noteOffset);
		}
		else
		{
			DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", hitmansHUD.iconP2.getCharacter());
	...
	override public function update(elapsed:Float)
	{
		...
		var shaderThing = LuaUtils.lua_Shaders;
	...
	// 	health -= (subtract-(ClientPrefs.data.cusualMode ? 0.15 : 0) * healthLoss)/(daNote.isSustainNote ? sustainDivider : 1); //now sustains gets 10 times less gain but still drain
	// 	songScore -= 10;
	// 	if (note != null && !daNote.isSustainNote){
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		health -= (daNote.missHealth-(ClientPrefs.data.casualMode ? 0.15 : 0) * healthLoss)/(daNote.isSustainNote ? sustainDivider : 1); //now sustains gets 10 times less gain but still drain
	...
	function goodNoteHit(note:Note):Void
	{
		...
		if(!note.hitCausesMiss) {
			...
			health += (note.hitHealth+(Note.edwhakIsPlayer ? 0.012 : (ClientPrefs.data.casualMode ? 0.007 : 0)) * healthGain)/(note.isSustainNote ? sustainDivider : 1); //now sustains gets 10 times less gain but still gain
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null)
	{
		...
		var usedPractice:Bool = (practiceMode || cpuControlled || !notITGMod || ClientPrefs.data.casualMode);
	...
	public function onCheckPoint(strumTime:Float, hideTell:Bool = false, manual:Bool = false){
		...
		if(strumTime > previousCheckpointTime){
			...
			if(!hideTell){
				...
				variables.set("checkpoint!", FlxTween.tween(checkpointSprite, {alpha : 0.275}, 0.1, {ease: FlxEase.linear,
					onComplete: function(twn:FlxTween) {
						variables.remove("checkpoint!");
						variables.set("checkpoint!", FlxTween.tween(checkpointSprite, {alpha : 0.0}, 0.45, {ease: FlxEase.linear,
							onComplete: function(twn:FlxTween) {
								variables.remove("checkpoint!");
							}
						}));
	...
	function modchartTweenCancel(tag:String){
		if(variables.exists(tag)){
			cast(variables.get(tag), FlxTween).cancel();
