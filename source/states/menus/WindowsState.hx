...
class WindowsState extends MusicBeatState
{
	...
	var gameNickname:String = ClientPrefs.data.userName;
	...
	*/
	override public function create():Void
	{
		...
		trace(TitleState.muteKeys, TitleState.volumeDownKeys, TitleState.volumeUpKeys);
		//changing states incase if nickname != null
		if(ClientPrefs.data.isLogged && ClientPrefs.data.userName != '' && !testing) {
			...
			gameNickname = ClientPrefs.data.userName;
			...
			if(gameText.alpha == 1 && playerStep > 2){
				...
				if(noText.alpha > 0.5 && yesText.alpha > 0.5){
					...
					if(FlxG.keys.justPressed.ENTER) {
						if(onYes) {
							FlxG.sound.play(Paths.sound('scrollMenu'), 0.6);
							ClientPrefs.data.casualMode = true;
							ClientPrefs.saveSettings();
							playerStep += 1;
							gameTalks();
						} else {
							FlxG.sound.play(Paths.sound('scrollMenu'), 0.6);
							ClientPrefs.data.casualMode = false;
							ClientPrefs.saveSettings();
							playerStep += 1;
							gameTalks();
						}
				...
				if (FlxG.keys.justPressed.ENTER)
					{
						if(wordText.text.length > 2 && wordText.text != ''){
							if (!testing && !specialGuests.contains(wordText.text.toLowerCase())){
								...
								ClientPrefs.data.userName = wordText.text;
								ClientPrefs.data.isLogged = true;
								ClientPrefs.saveSettings();
							}
	...
	function cubeEffect()
	{
		if(!ClientPrefs.data.lowQuality) {
			...
			bigCube.antialiasing = ClientPrefs.data.antialiasing;
			...
			mediumCube.antialiasing = ClientPrefs.data.antialiasing;
			...
			smallCube.antialiasing = ClientPrefs.data.antialiasing;
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			var babyArrow:StrumNote = new StrumNote(STRUM_X_MIDDLESCROLL, strumLine.y, i, player, 'Skins/Notes/'+ClientPrefs.data.notesSkin[0]+'/NOTE_assets', 'shared');
			babyArrow.alpha = 0;
