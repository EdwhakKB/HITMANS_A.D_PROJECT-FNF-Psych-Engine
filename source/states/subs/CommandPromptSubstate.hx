...
class CommandPromptSubstate extends MusicBeatSubstate
{
	...
	*/
	...
	override function create()
	{
		...
		promptBG.antialiasing = ClientPrefs.data.antialiasing;
		...
		promptFrame.antialiasing = ClientPrefs.data.antialiasing;
		...
		// developerImage.antialiasing = ClientPrefs.data.antialiasing;
		...
		// if(ClientPrefs.data.edwhakMode) {
		...
		// } else if(ClientPrefs.data.developerMode) {
		// 	// infoText.text = "Wellcome Developer - " + ClientPrefs.data.userName + "! what are we going to test today?";
		// 	// promptText.text = "Hitmans Corporation [DEVELOPER]\nHave a nice day!";
		// 	developerImage.visible = true;
		// }

		currentName = ClientPrefs.data.userName; //to have a backUp of this in case player want play a nonDev mode edition (dev mode will be everywhere LOL)
    }
	...
	function applyCommand(command:String)
	{
		switch(command){
			...
			case 'login':
				if (!changingUserName){
					if(ClientPrefs.data.userName == 'Guess') {
						changingUserName = true;
						updatePromptText('Please type your new username');
					} else {
						updatePromptText('You are already logged in as '+ ClientPrefs.data.userName +' try "Logout" to change userName');
					}
				}
			case 'logout':
				if(!changingUserName) {
					if(ClientPrefs.data.userName != 'Guess' && !ClientPrefs.edwhakMode && !ClientPrefs.developerMode) {
						updatePromptText("You was successfully logged out");
						ClientPrefs.data.userName = 'Guess';
					} else if(ClientPrefs.data.userName != 'Guess' && ClientPrefs.edwhakMode || ClientPrefs.developerMode) {
						updatePromptText("Are you sure you want to logout? "+ ClientPrefs.data.userName + " You must login again");
					} else if(ClientPrefs.data.userName == 'Guess') {
		...
		if(!stopWorking){
			...
			if(FlxG.keys.justPressed.ENTER) {
				...
				if(!changingUserName && !resetWarning && cmdCheck != '')
					checkCommand(keyInput.toLowerCase()); //i can check if it is a command or something else such as ClientPrefs.data or something else
