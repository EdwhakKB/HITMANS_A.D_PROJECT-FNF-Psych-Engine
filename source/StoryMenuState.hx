...
class StoryMenuState extends MusicBeatState {
	...
	var daHud:FlxSprite;
	var daStart:FlxSprite;
	var daExit:FlxSprite;
	var daSettings:FlxSprite;
	...
	override function create() {
		...
		WMText = new FlxText(scoreText.x - 10, scoreText.y, 0, "WEEK MISSES: 49324858", 36);
		WMText.setFormat("VCR OSD Mono", 32);
		...
		var bgYellow:FlxSprite = new FlxSprite(0, 56).makeGraphic(FlxG.width, 406, 0xFFF9CF51);
		...
		DiscordClient.changePresence("Menu - StoryMode", null);
		#end
		...
		rightArrow = new FlxSprite(leftArrow.x + 336, leftArrow.y);
	...
	override function update(elapsed:Float) {
		...
		WMText.text = "WEEK MISSES:" + intendedWM;
		...
		if (FlxG.mouse.overlaps(daStart)) {
			if (FlxG.mouse.justPressed) {
				selectWeek();
			}
			daStart.color = 0x494949;
		} else {
			daStart.color = 0xFFFFFF;
		}

		if (FlxG.mouse.overlaps(daExit)) {
			if (FlxG.mouse.justPressed) {
				MusicBeatState.switchState(new MainMenuState());
				FlxG.sound.play(Paths.sound('cancelMenu'));
			}
			daExit.color = 0x494949;
		} else {
			daExit.color = 0xFFFFFF;
		}

		if (FlxG.mouse.overlaps(daSettings)) {
			if (FlxG.mouse.justPressed) {
				persistentUpdate = false;
				openSubState(new GameplayChangersSubstate());
				FlxG.sound.play(Paths.sound('scrollMenu'));
			}
			daSettings.color = 0x494949;
		} else {
			daSettings.color = 0xFFFFFF;
		}
	...
	function changeDifficulty(change:Int = 0):Void {
		...
		if (sprDifficulty.graphic != newImage) {
			...
			sprDifficulty.x = leftArrow.x + 40;
	...
	function updateText() {
		...
		#if !switch
		intendedScore = Highscore.getWeekScore(loadedWeeks[curWeek].fileName, curDifficulty);
		intendedWM = PlayState.campaignMisses;
		#end
	}
}