...
// import modchart.*;
import managers.LuaTweenManager;
...
import play.HazardAFT_Capture as AFT_capture;
...
class EditorPlayState extends MusicBeatState
{
	...
	// var modchartedSongs:Array<String> = []; PUT THE PlayState.SONG NAME HERE IF YOU WANT TO USE THE ANDROMEDA MODIFIER SYSTEM!!
	...
	public static var forcedAScroll:Bool = false; //if forced then it should disable "ClientPrefs.data" stuff
	...
	override public function create()
	{
		...
		EditorPauseSubState.songName = null; //Reset to default
		...
		// useModchart = modchartedSongs.contains(PlayState.SONG.song.toLowerCase());
		...
		strumLine = new FlxSprite(!forcedAScroll ? (ClientPrefs.data.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X) : 
												   (if (forceRightScroll && !forceMiddleScroll) STRUM_X 
												   else if (forceMiddleScroll && !forceRightScroll) STRUM_X_MIDDLESCROLL 
												   else ClientPrefs.data.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X), 
		...
		// PlayState.SONG SPECIFIC SCRIPTS
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED)
		...
		if (EditorPauseSubState.songName != null) {
			precacheList.set(EditorPauseSubState.songName, 'music');
		...
		if (!inResultsScreen){
			DiscordClient.changePresence(detailsText, PlayState.SONG.song + " (" + storyDifficultyText + ")", hitmansHUD.iconP2.getCharacter());
		}
		#end
	...
	public function spawnDialogue()
	{
		var file:String = "";
		var music:String = "";
		switch (Paths.formatToSongPath(PlayState.SONG.song).toLowerCase())
		{
			case 'operating':
				file = 'dialogue';
		}
	...
	function startCharacterScripts(name:String)
	{
		...
		#else
		luaFile = Paths.getSharedPath(luaFile);
		if(OpenFLAssets.exists(luaFile)) doPush = true;
		#end
		...
		if(doPush)
		{
			...
			if(doPush) new EditorLua(luaFile);
		}
		#end
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				notes.forEachAlive(function(note:Note) {
					if(ClientPrefs.data.opponentStrums || note.mustPress)
					{
						...
						if(ClientPrefs.data.middleScroll && !note.mustPress && !forceRightScroll || forceMiddleScroll) {
							note.alpha *= 0.35;
						}
					}
				});
	...
	public function startSong():Void
	{
		...
		if((PlayState.SONG.song.toLowerCase() == "system-reloaded" || PlayState.SONG.song.toLowerCase() == "metakill" ) && storyDifficulty == 1){
		...
		if (!inResultsScreen){
			DiscordClient.changePresence(detailsText, PlayState.SONG.song + " (" + storyDifficultyText + ")", hitmansHUD.iconP2.getCharacter(), true, songLength);
		}
		#end
	...
	private function generateSong(dataPath:String):Void
	{
		...
		{
			case "multiplicative":
				songSpeed = PlayState.SONG.speed * ClientPrefs.getGameplaySetting('scrollspeed', 1);
			case "constant":
	...
	override function closeSubState()
	{
		...
		if (EditorPauseSubState.goToOptions){
			if (EditorPauseSubState.goBack)
			{
				EditorPauseSubState.goToOptions = false;

				openSubState(new EditorPauseSubState(boyfriend.getScreenPosition().x, boyfriend.getScreenPosition().y));
				EditorPauseSubState.goBack = false;
			}
			else
			{
				openSubState(new OptionsMenu(true));
			}
		}else if (EditorPauseSubState.goToModifiers)
		{
			trace("pause thingyt");
			if (EditorPauseSubState.goBackToPause)
			{
				trace("pause thingyt");
				EditorPauseSubState.goToModifiers = false;

				openSubState(new EditorPauseSubState(boyfriend.getScreenPosition().x, boyfriend.getScreenPosition().y));
				EditorPauseSubState.goBackToPause = false;
			}
			else
			{
			...
			if (!inResultsScreen){
				if (startTimer != null && startTimer.finished)
				{
					DiscordClient.changePresence(detailsText, PlayState.SONG.song + " (" + storyDifficultyText + ")", hitmansHUD.iconP2.getCharacter(), true, songLength - Conductor.songPosition - ClientPrefs.data.noteOffset);
				}
				else
				{
					DiscordClient.changePresence(detailsText, PlayState.SONG.song + " (" + storyDifficultyText + ")", hitmansHUD.iconP2.getCharacter());
	...
	override public function onFocusLost():Void
	{
		...
		{
			if (!inResultsScreen){
				DiscordClient.changePresence(detailsPausedText, PlayState.SONG.song + " (" + storyDifficultyText + ")", hitmansHUD.iconP2.getCharacter());
			}
		}
		#end
	...
	override public function update(elapsed:Float)
	{
		...
		switch (modChartEffect){
			...
			case 1:
				...
				if(PlayState.SONG.song.toLowerCase()=="her" && curBeat < 608){
		...
		var shaderThing = EditorLua.lua_Shaders;
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String, strumTime:Float) {
		switch(eventName) {
			...
			case 'Set Property':
				...
				if(killMe.length > 1) {
					EditorLua.setVarInArray(EditorLua.getPropertyLoopThingWhatever(killMe, true, true), killMe[killMe.length-1], value2);
				} else {
					EditorLua.setVarInArray(this, value1, value2);
				}
				//modchart variables
	...
	public function moveCamera(isDad:Bool)
	{
		...
		else
		{
			...
			if (Paths.formatToSongPath(PlayState.SONG.song) == 'tutorial' && cameraTwn == null && FlxG.camera.zoom != 1)
			{
	...
	function tweenCamIn() {
		if (Paths.formatToSongPath(PlayState.SONG.song) == 'tutorial' && cameraTwn == null && FlxG.camera.zoom != 1.3) {
	...
	public function rating():Void
	{
		...
		var oldBest:Int = Highscore.getScore(PlayState.SONG.song, storyDifficulty);
		...
		#if desktop
			DiscordClient.changePresence("Results - " + detailsText, PlayState.SONG.song + " (" + storyDifficultyText +")" + "Score:" + Math.round(songScore), hitmansHUD.iconP2.getCharacter());
		#end
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		if (!cpuControlled && startedCountdown && !paused && !falsePaused && key > -1 && (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || controls.controllerMode))
		{
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if (!daNote.isSustainNote){
			...
			if (!edwhakIsEnemy && !PlayState.SONG.bossFight){
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (!note.isSustainNote && !note.isHoldEnd)
			{
				...
				if (!edwhakIsEnemy && !PlayState.SONG.bossFight){
					hitmansHUD.ratingsBumpScaleOP();
					hitmansHUD.setRatingImageOP(note.strumTime - Conductor.songPosition + ClientPrefs.data.ratingOffset);
				}
				combo += 1;
	...
	override function destroy() {
		...
		luaArray = [];
		EditorLua.killShaders();

		#if hscript
		if(EditorLua.hscript != null) EditorLua.hscript = null;
		#end
	...
	override function sectionHit()
	{
		...
		if (PlayState.SONG.notes[curSection] != null)
		{
			...
			if (camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.data.camZooms && autoCamZoom) //need auto cam zoom to make this works :b
			{
			...
			setOnScripts('mustHitSection', PlayState.SONG.notes[curSection].mustHitSection);
			setOnScripts('altAnim', PlayState.SONG.notes[curSection].altAnim);
			setOnScripts('gfSection', PlayState.SONG.notes[curSection].gfSection);
		}
