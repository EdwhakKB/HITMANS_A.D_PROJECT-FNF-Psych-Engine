...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		DiscordClient.changePresence("Chart Editor", StringTools.replace(_song.song, '-', ' ') + " (" + (_song.mania + 1) + " key" + s_termination + ")");
		#end
	...
	function addChartingUI() {
		...
		playSoundDad = new FlxUICheckBox(check_mute_inst.x + 120, playSoundBf.y, null, null, 'Play Sound (Enemy notes)', 100,
			function() {
	...
	*/
	...
	function reloadGridLayer() {
		...
		if(foundNextSec)
		{
			var gridBlack:FlxSprite = new FlxSprite(gridBG.x, gridBG.height).makeGraphic(Std.int(GRID_SIZE + GRID_SIZE * Note.ammo[_song.mania] * 2), Std.int(nextGridBG.height), FlxColor.BLACK);
			gridBlack.alpha = 0.4;
